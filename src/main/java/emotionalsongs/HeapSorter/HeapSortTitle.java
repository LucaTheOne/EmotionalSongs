...
public class HeapSortTitle {
    ...
    void buildHeap(Song[] songsArray, int N, int i) {
        ...
        if (l < N && songsArray[l].compareTitle(songsArray[largest])>0) largest = l;

        // If right child is larger than largest so far
        if (r < N && songsArray[r].compareTitle(songsArray[largest])>0) largest = r;
